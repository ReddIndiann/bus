    ...
    <header className={`fixed w-full z-10 transition-colors duration-300 ${scrolled ? 'bg-black' : 'bg-black bg-opacity-50'} text-white`}>
      <div className="container mx-auto flex items-center justify-between py-4 px-4">
        ...
        <nav className={`md:block ${isOpen ? 'block' : 'hidden'}`}>
          <div className={`fixed inset-y-0 right-0 w-64 bg-black text-white shadow-lg z-50 transition-transform duration-300 ease-in-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} md:relative md:translate-x-0 md:w-auto md:bg-transparent md:shadow-none`}>
            {isOpen && (
              <button
                onClick={toggleMenu}
            ...
            <ul className="flex flex-col md:flex-row md:items-center md:space-x-10 mt-16 md:mt-0 space-y-6 md:space-y-0 p-8 md:p-2">
              {menuItems.map((item) => (
                <li key={item.href} className="md:border-none">
                  <Link href={item.href} passHref>
                    <div
                      className={`text-lg md:text-base hover:text-green-500 cursor-pointer transition-colors duration-200 ${isActive(item.href) ? 'text-green-500 font-bold' : ''}`}
                      onClick={() => isOpen && setIsOpen(false)}
                    >
                      {item.label}
                    </div>
              ...
              <li className="mt-8 md:mt-0">
                <Link href="/Contact" passHref>
                  <button
                    className="bg-green-500 text-white px-6 py-2 rounded-full hover:bg-green-600 transition-colors duration-200 text-base"
                    onClick={() => isOpen && setIsOpen(false)}
                  >
                    Contact Us
                  </button>
